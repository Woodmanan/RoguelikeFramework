...
[Priority(10)]
public class VortexHot : Effect
{
    public int duration;
    public float damagePerTurn;
    [Range(0, 100)]
    public float percentManaTransfer;
    ...
    {
        connectedTo.monster.Damage(credit, damagePerTurn, DamageType.MAGICAL | DamageType.FIRE, DamageSource.EFFECT);
        duration--;
        if (duration == 0)
        {
            Disconnect();
