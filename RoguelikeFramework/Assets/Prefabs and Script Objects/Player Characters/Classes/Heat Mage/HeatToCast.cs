...
//Quick and dirty spell effect that prevents casting unless heat is at a certain level.

[Group("Class Effects/Heat Mage")]
[Priority(10)]
public class HeatToCast : Effect
{
    public float minimumNeeded;
    ...
    {
        if (connectedTo.monster.currentStats[Resources.HEAT] < minimumNeeded)
        {
            available = false;
        }
        
        if (connectedTo.monster.currentStats[Resources.HEAT] > maximumAllowed)
        {
            available = false;
        }
    }
