...
[Priority(499)]
public class GreaterNecrotize : Effect
{
    public int duration;

    [SerializeReference]
    public Effect GreaterUndeadEffect;

    [SerializeReference]
    public Effect LichPassive;

    [Header("Lich attributes")]
    [SerializeField]
    Sprite PlayerLichImage;
    [SerializeField]
    LocalizedString LichName;
    ...
    public override string GetUISubtext() { return duration.ToString(); }
    ...
    {
        GreaterNecrotize necro = other as GreaterNecrotize;
    ...
    //Called after this monster has registered it
    //s death.
    public override void OnPostDeath()
    {
        if (!connectedTo.monster.tags.MatchAnyTags(new RogueTag("Monster.Undead"), TagMatch.Parental))
        {
            ...
            //Remove and replace old species attribute
            Effect effect = GreaterUndeadEffect.Instantiate();
            ...
            if (connectedTo.monster == Player.player)
            {
                ...
                //Lich time!
                player.gameObject.name = $"Lich {{{Player.player.gameObject.name}}}";
                ...
                //TODO: Cool particle effect here?
            }
        }
        Disconnect();
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnTurnEndGlobal.AddListener(499, OnTurnEndGlobal);

        c.OnPostDeath.AddListener(499, OnPostDeath);
