...
[Priority(10)]
public class GreaterUndead : Effect
{
    [HideInInspector]
    public int numberOfKills;
    public float maxNumberOfKills;

    public float manaPerKill;

    public float lowerManaPercent;
    public float upperManaPercent;

    public float lowerHealthPercent;
    public float upperHealthPercent;
    ...
    {
        numberOfKills = Mathf.Min(numberOfKills + 1, Mathf.RoundToInt(maxNumberOfKills));
        connectedTo.monster.AddBaseStat(MANA, manaPerKill);
    }

    //Called often, whenever a monster needs up-to-date stats.
    public override void RegenerateStats(ref Stats stats)
    {
        stats[MAX_HEALTH] *= Mathf.Lerp(lowerHealthPercent, upperHealthPercent, numberOfKills / maxNumberOfKills) / 100;
        stats[MAX_MANA]   *= Mathf.Lerp(lowerManaPercent, upperManaPercent, numberOfKills / maxNumberOfKills) / 100;
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        connectedTo.OnPostDeath.AddListener(100, OnPostDeath);
