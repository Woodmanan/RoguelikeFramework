...
{
    public static EffectResourceTable table;

    public ResourceEffectKey key;
    ...
    public override Effect Instantiate()
    {
        if (!table)
        {
            table = UnityEngine.Resources.Load<EffectResourceTable>("Standard Effects");
            if (table == null)
            {
                Debug.LogError("No resource table with the name 'Standard Effects' found!");
                return null;
            }
        }

        return table?.GetEffect(key)?.Instantiate();
    }
    ...
    {
        connectedTo = c;
        connectedTo.OnGenerateLocalizedString.AddListener(100, OnGenerateLocalizedString);
        ...
        connectedTo.OnStartAttackTarget.AddListener(100, OnStartAttackTarget);
