...
[Group("Branch/Hive")]
[Priority(10)]
public class SummonGuards : Effect
{
    public int summonEvery;
    int currentCooldown;
    ...
    {
        currentlySummoned = new List<Monster>();
        currentCooldown = summonEvery;
    }
    ...
    public override void OnTurnEndGlobal()
    {
        ...
        {
            currentlySummoned = currentlySummoned.Where(x => x != null && !x.IsDead()).ToList();
            currentCooldown = summonEvery;
            int numToSummon = numToSpawn.Evaluate();
            if (maxSpawn > 0)
            {
                int allowedToSpawn = maxSpawn - currentlySummoned.Count;
    ...
    public void Summon()
    {
        List<Vector2Int> targets = connectedTo.monster.view.GetVisibleTiles(Map.current).ToList();
        while (targets.Count > 0)
        {
            int index = RogueRNG.Linear(0, targets.Count);
            Vector2Int pos = targets[index];
