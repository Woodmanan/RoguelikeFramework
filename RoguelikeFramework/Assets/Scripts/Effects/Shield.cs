...
public class Shield : Effect
{
    ...
    public float startingShieldAmount;
    public float maxShieldAmount;
    public DamageType blockedDamageTypes;
    ...
    {
        return Mathf.CeilToInt(currentShieldAmount).ToString();
    }
    ...
    public override float GetUIFillPercent()
    {
        if (maxDuration > 0)
        {
            return 1f - ((float)currentDuration) / maxDuration;
        }
    ...
    {
        Shield otherShield = other as Shield;
        if (otherShield != null)
        {
            if (otherShield.blockedDamageTypes == this.blockedDamageTypes)
            {
                startingShieldAmount += otherShield.currentShieldAmount;
                otherShield.Disconnect();
            }
        }
    }
    ...
    public override void OnTurnEndGlobal()
    {
        ...
        currentShieldAmount = Mathf.Clamp(currentShieldAmount + regenPerTurn, 0, maxShieldAmount);
    ...
    {
        if ((damageType & blockedDamageTypes) > 0)
        {
            float amountToBlock = Mathf.Min(damage, currentShieldAmount);
            damage -= amountToBlock;
            currentShieldAmount -= amountToBlock;
            if (currentShieldAmount <= 0)
            {
                Disconnect();
            }
