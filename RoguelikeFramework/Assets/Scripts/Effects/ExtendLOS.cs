...
/*
 * This status effect allows the connected monster to see any tile within range,
 * regardless of if that tile would be visibile from normal LOS.
 */

//[EffectGroup("Sample Group/Sample Subgroup")]
[Priority(10)]
public class ExtendLOS : Effect
{
    public int duration;
    public int range;
    ...
    {
        int usableRadius = Mathf.Min(view.radius, range);

        Vector2Int start = view.origin - Vector2Int.one * usableRadius;
        Vector2Int viewStart = (view.radius - usableRadius) * Vector2Int.one;

        for (int i = 0; i < (usableRadius * 2 + 1); i++)
        {
            for (int j = 0; j < (usableRadius * 2 + 1); j++)
            {
                Vector2Int mapLoc = new Vector2Int(i + start.x, j + start.y);
                Vector2Int viewLoc = new Vector2Int(i + viewStart.x, j + viewStart.y);

                if (Map.current.ValidLocation(mapLoc))
                {
                    view.definedArea[viewLoc.x, viewLoc.y] = true;
                }
            }
        }
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnGenerateLOSPreCollection.AddListener(10, OnGenerateLOSPreCollection);
