...
[Group("Core")]
[Priority(10)]
public class ApplyPipFromDamage : Effect
{
    public PipPairing[] pairs;
    ...
    {
        foreach (PipPairing pair in pairs)
        {
            if ((pair.type & damageType) > 0)
            {
                PipEffect effect = pair.effect.Instantiate() as PipEffect;
                effect.stackCount = Mathf.CeilToInt(damage);
