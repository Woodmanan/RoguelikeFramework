...
public class Ice : PipEffect
{
    ...
    {
        int blockedDamage = Mathf.CeilToInt(Mathf.Min(damage, stackCount));
        damage = Mathf.Max(0, damage - blockedDamage);
        stackCount = stackCount - blockedDamage;
        if (stackCount == 0)
        {
            Disconnect();
        }
