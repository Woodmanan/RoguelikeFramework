...
public class Poison : PipEffect
{
    ...
    public override void OnStacksGained(int delta)
    {
        base.OnStacksGained(delta);
        Debug.Log($"Poison gained {delta} stacks!");
    }
    ...
    {
        Debug.Log($"Poison pip is applying {stackCount} damage!");
        connectedTo.monster.Damage(credit, stackCount, DamageType.NONE, DamageSource.EFFECT);
        TickDown();
    }
