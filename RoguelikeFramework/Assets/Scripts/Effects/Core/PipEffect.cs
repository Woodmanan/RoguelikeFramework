...
public class PipEffect : Effect
{
    ...
    {
        //Using GetType since this needs to work for inheriting types as well.
        if (addThisEffect && other.GetType() == GetType())
        {
            PipEffect asPip = other as PipEffect;
            if (asPip != null)
            {
                asPip.stackCount += this.stackCount;
                asPip.OnStacksGained(this.stackCount);
                addThisEffect = false;
            }
        }
    }
