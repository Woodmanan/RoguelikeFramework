...
[Group("Teleportation")]
[Priority(10)]
public class TeleportAnchor : Effect
{
    //public static Dictionary<Monster, TeleportAnchor> anchors = new Dictionary<Monster, TeleportAnchor>();
    public static TeleportAnchor anchor;
    ...
    {
        options = LOS.LosAt(Map.current, connectedTo.monster.location, radius);
        DropGlobalAnchor();
        anchor = this;
        //anchors.Add(connectedTo.monster, this);
    }

    public void DropGlobalAnchor()
    {
        if (anchor == this)
        {
            anchor = null;
        ...
        /*
        if (anchors.ContainsKey(connectedTo.monster))
        {
            anchors.Remove(connectedTo.monster);
        }*/
    }
    ...
    public List<Vector2Int> GetValidLocations()
    {
        return options.GetVisibleTiles(Map.current)
                                        .Where(x => Map.current.GetTile(x).IsOpen())
                                        .ToList();
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnGenerateLOSPostCollection.AddListener(10, OnGenerateLOSPostCollection);
