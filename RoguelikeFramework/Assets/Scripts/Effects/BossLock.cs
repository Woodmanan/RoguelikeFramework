...
//Locks the doors, unlocks on death of this creature

[Group("Monster Only")]
[Priority(1000)]
public class BossLock : Effect
{
    bool locked = false;
    ...
    public List<Stair> GetAllStairs()
    {
        List<Stair> stairs = new List<Stair>();
        foreach (LevelConnection connect in LevelLoader.singleton.world.connections)
        {
            if (connect.fromLevel == Map.current.index)
            {
                if (connect.fromStair)
                {
                    stairs.Add(connect.fromStair);
                }
            }
            if (connect.toLevel == Map.current.index)
            {
                if (connect.toStair)
                {
                    stairs.Add(connect.toStair);
                }
            }
        }
    ...
    public void Lock()
    {
        foreach (Stair stair in GetAllStairs())
        {
            stair.Lock();
        }
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnTurnEndGlobal.AddListener(1000, OnTurnEndGlobal);
