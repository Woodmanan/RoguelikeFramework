...
[Priority(10)]
public class PerfectTiming : Effect
{
    ...
    public int activeWindowMaxDuration;

    [HideInInspector]
    public bool isActive;
    [HideInInspector]
    public int activeWindowDuration;

    public int cooldownReductionOnKill;

    [SerializeReference]
    TargetingAnimation animOnActive;
    ...
    {
        startsAt = RogueRNG.Linear(activeWindowMaxDuration, maxDuration - 1);
    ...
    public override void OnTurnEndGlobal()
    {
        ...
        else if (currentDuration == startsAt)
        {
            isActive = true;
            activeWindowDuration = activeWindowMaxDuration;

            { //TODO: Improve animation pipeline to support something like this better.
                TargetingAnimation toPlay = animOnActive.Instantiate();
                toPlay.origin = connectedTo.monster.location;
    ...
    public override void OnDeath(ref Monster killer)
    {
        if (isActive)
        {
            foreach (Ability ability in credit.abilities.GetAbilitiesAsEnumerable())
            {
                ability.currentCooldown = Mathf.Max(0, ability.currentCooldown - cooldownReductionOnKill);
            }
        }
    }
