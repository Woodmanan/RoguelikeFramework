...
//Status effect that summons husks on this target after a few turns

[Group("Branch/Nightmare")]
[Priority(10)]
public class SummonHusksTimed : Effect
{
    public int numTurnsUntilSummon;
    public int numHusksToSummon;
    public Monster huskPrefab;
    ...
    public void CallSummon()
    {
        ...
        {
            SpawnAt(position, credit, connectedTo.monster);
        }

        if (positions.Count > 0)
        {
            Debug.Log($"Console: You hear a swarm incoming! Run!");
        }
        else
        {
            Debug.Log($"Console: Nothing responds to the call...");
        }
    }

    public List<Vector2Int> GetNearestUnexploredTiles(Monster target, int count)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            Path path = Pathfinding.CreateDjikstraPath(target.location, goals);
            ...
            {
                //Clear goal out, add it to found
                goals.Remove(path.destination);
                found.Add(path.destination);
            }
        }
