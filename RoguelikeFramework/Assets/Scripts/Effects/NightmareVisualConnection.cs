...
[Priority(10)]
//After x turns of continuous visual connection, add time to the nightmare counter
public class NightmareVisualConnection : Effect
{
    ...
    public override void OnTurnStartLocal()
    {
        if (credit != null && connectedTo.monster.view.GetVisibleMonsters(connectedTo.monster).Contains(credit))
        {
            ...
            {
                Debug.Log("Console: You feel a deep sense of dread. The nightmare grows much closer.");
                AddTicksToNightmare();
                Disconnect();
            }
        }
        ...
        {
            Debug.Log("The connection is broken!");
            Disconnect();
        }
    }

    public void AddTicksToNightmare()
    {
        foreach (DungeonSystem system in Map.current.mapSystems)
        {
            ...
            if (nightmare != null)
            {
                for (int i = 0; i < numberOfTicksToAdd; i++)
                {
                    nightmare.TickNightmareSummonOnly();
                }
            }
        }
    }
