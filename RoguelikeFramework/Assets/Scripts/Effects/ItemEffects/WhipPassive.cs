...
/*
 * Slightly unique weapon passive - this attaches to the player, rather than the weapon. This is necessary
 * to allow the weapon to intercede in movement requests to perform it's special attack!
 */

[Group("Weapon Passives")]
[Priority(10)]
public class WhipPassive : Effect
{
    public int range = 2;
    ...
    {
        if (canMove) //Prevent stacking!
        {
            //Check for regular attack
            if (Map.current.ValidLocation(newLocation))
            {
                RogueTile adjacentTile = Map.current.GetTile(newLocation);
                ...
                {
                    searchLocation = searchLocation + offset;
                    if (Map.current.ValidLocation(searchLocation))
                    {
                        ...
                        else if (tile.blocksProjectiles)
                        {
                            break;
                        }
                    }
    ...
    //public override void OnGenerateArmedAttacks(ref AttackAction action, ref List<Weapon> primaryWeapons, ref List<Weapon> secondaryWeapons) {}
    ...
    //public override void OnGenerateUnarmedAttacks(ref AttackAction action, ref List<EquipmentSlot> slots) {}
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnMoveInitiated.AddListener(10, OnMoveInitiated);
