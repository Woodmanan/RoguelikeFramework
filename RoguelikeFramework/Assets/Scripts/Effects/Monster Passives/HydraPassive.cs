...
[Group("Monster Only")]
[Priority(300)]
public class HydraPassive : Effect
{
    public EquipmentSlot headSlot;

    public RandomNumber startingNumber;

    List<EquipmentSlot> generatedHeads;

    public DamageType pauseOn;
    ...
    public void AddHeads(int count = 1)
    {
        for (int i = 0; i < count; i++)
        {
            EquipmentSlot instance = headSlot.Instantiate();
            generatedHeads.Add(instance);
            connectedTo.monster.equipment.equipmentSlots.Add(instance);
            currentNumber++;
        }

        Debug.Log($"It grows {count}! {currentNumber}");

        UpdateSprite();
    }

    public void RemoveHeads(int count = 1)
    {
        for (int i = 0; i < count; i++)
        {
            connectedTo.monster.equipment.equipmentSlots.Remove(generatedHeads[generatedHeads.Count - 1]);
            generatedHeads.RemoveAt(generatedHeads.Count - 1);
            currentNumber--;
            if (currentNumber == 0)
            {
                //DIE!
                connectedTo.monster.Damage(killCredit, 999, DamageType.TRUE, DamageSource.EFFECT);
                Disconnect();
                return;
            }
        }

        Debug.Log($"It loses {count} head. {currentNumber}");
        UpdateSprite();
    }
    ...
    {
        generatedHeads = new List<EquipmentSlot>();
        currentNumber = 0;

        AddHeads(startingNumber.Evaluate());
    }
    ...
    {
        if ((damageType & pauseOn) > 0)
        {
            dealtFireDamage = true;
        }
    }
    ...
    public override void OnAfterPrimaryAttackTarget(ref Weapon weapon, ref AttackAction action, ref AttackResult result)
    {
        ...
        {
            killCredit = action.caller;
            CheckHit(weapon.primary);
        }
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnAfterPrimaryAttackTarget.AddListener(300, OnAfterPrimaryAttackTarget);

        c.OnBeforeSecondaryAttackTarget.AddListener(300, OnBeforeSecondaryAttackTarget);

        c.OnAfterSecondaryAttackTarget.AddListener(300, OnAfterSecondaryAttackTarget);

        c.OnAfterUnarmedAttackTarget.AddListener(300, OnAfterUnarmedAttackTarget);
