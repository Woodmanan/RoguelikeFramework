...
[Group("Species")]
[Priority(10)]
public class TrollPassive : Effect
{
    public float baseRegenPerTurn;
    public float extraRegenPerLevel;
    public DamageType stoppedBy;
    public float numTurnsHaltOnWeakness;
    ...
    public override void OnTurnEndGlobal()
    {
        ...
        {
            connectedTo.monster.Heal(baseRegenPerTurn + connectedTo.monster.level * extraRegenPerLevel, false);
        }
    }
    ...
    {
        if (damage > 0 && ((damageType & stoppedBy) > 0))
        {
            currentWait = numTurnsHaltOnWeakness;
            Debug.Log($"It burns! Your healing stops for {currentWait} turns.");
        }
    }
