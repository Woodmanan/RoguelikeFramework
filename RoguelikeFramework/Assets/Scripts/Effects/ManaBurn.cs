...
public class ManaBurn : Effect
{
    ...
    [Range(0, 100)]
    public float percentageManaToDamage;
    public DamageType typeOfBurn;
    ...
    {
        float burn = Mathf.Min(target.baseStats[Resources.MANA], manaLost);
        target.AddBaseStat(Resources.MANA, -burn);
        float damage = manaLost * percentageManaToDamage * 0.01f;
        target.Damage(connectedTo.monster, damage, typeOfBurn, DamageSource.MELEEATTACK);
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnEndUnarmedAttack.AddListener(10, OnEndUnarmedAttack);
