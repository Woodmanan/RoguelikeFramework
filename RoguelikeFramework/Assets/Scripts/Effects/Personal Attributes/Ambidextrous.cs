...
//TODO: Update this class in a major way with the limb rework
/*
 * This class is essentially going to be a lie. For now, it'll just instantly convert
 * all your limbs, but won't do anything else worthwhile.
 */

[Group("Personal Attributes")]
[Priority(10)]
public class Ambidextrous : Effect
{
    ...
    {
        EquipmentSlot primary = new EquipmentSlot();

        foreach (EquipmentSlot slot in connectedTo.monster.equipment.equipmentSlots)
        {
        ...
        foreach (EquipmentSlot slot in connectedTo.monster.equipment.equipmentSlots)
        {
            if (slot.type.Contains(EquipSlotType.SECONDARY_HAND) && !slot.type.Contains(EquipSlotType.PRIMARY_HAND))
            {
                slot.type = primary.type;
                slot.unarmedAttack = primary.unarmedAttack;
                slot.slotName = primary.slotName;
            }
        }
    }
