...
[Priority(10)]
public class PerfectReflexes : Effect
{
    ...
    public override void OnUnarmedAttackResult(ref EquipmentSlot slot, ref AttackAction action, ref AttackResult result)
    {
        if (result == AttackResult.HIT)
        {
            if (currentDodges > 0)
            {
                action.target.Damage(action.caller, damagePerStack * currentDodges, DamageType.TRUE, DamageSource.EFFECT);
                currentDodges = 0;
            }
        }
    }
    ...
    public override void OnAfterPrimaryAttackTarget(ref Weapon weapon, ref AttackAction action, ref AttackResult result)
    {
        ...
        {
            currentDodges = Mathf.Clamp(currentDodges + 1, 0, maxDodges);
        }
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnAfterPrimaryAttackTarget.AddListener(10, OnAfterPrimaryAttackTarget);

        c.OnAfterSecondaryAttackTarget.AddListener(10, OnAfterSecondaryAttackTarget);

        c.OnAfterUnarmedAttackTarget.AddListener(10, OnAfterUnarmedAttackTarget);
