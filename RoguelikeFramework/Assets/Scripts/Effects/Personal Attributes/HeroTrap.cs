...
[Priority(10)]
public class HeroTrap : Effect
{
    float currentDifference = 1.0f;

    public float scalingHealthMultiple;

    public float damagePerDifference;
    ...
    {
        currentDifference = action.target.GetEstimatedStrength() / action.caller.GetEstimatedStrength();
        currentDifference *= Mathf.Lerp(damagePerDifference, 1, action.caller.baseStats[Resources.HEALTH] / action.caller.currentStats[Resources.MAX_HEALTH]);

        Debug.Log($"The current monster is {currentDifference} times stronger than the player");
    ...
    {
        if (result == AttackResult.HIT && currentDifference > 0)
        {
            action.target.Damage(action.caller, damagePerDifference * currentDifference, DamageType.TRUE, DamageSource.EFFECT);
        }
    }
    ...
    public override void OnSecondaryAttackResult(ref Weapon weapon, ref AttackAction action, ref AttackResult result)
    {
        if (result == AttackResult.HIT && currentDifference > 0)
        {
            action.target.Damage(action.caller, damagePerDifference * currentDifference / 2, DamageType.TRUE, DamageSource.EFFECT);
        }
    }
    ...
    public override void Connect(Connections c)
    {
        ...
        c.OnStartAttack.AddListener(10, OnStartAttack);
