...
[CustomPropertyDrawer(typeof(GodScoringOperator))]
public class ScoringPropertyDrawer : PropertyDrawer
{
    const float weightSize = 60f;
    ...
    {
        SelectImplementationAttribute typeAttribute = new SelectImplementationAttribute(typeof(GodScoringOperator));
        

        TryCacheTypesLogic.Execute(editorData, typeAttribute);
        TryCacheGodScoringOperatorNamesLogic.Execute(editorData, typeAttribute);
        ...
        weight.floatValue = EditorGUI.FloatField(popupRect, weight.floatValue);
...
public static class TryCacheGodScoringOperatorNamesLogic
{
    //GodScoringOperator-only version
